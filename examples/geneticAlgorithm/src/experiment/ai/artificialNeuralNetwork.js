...
class ArtificialNeuralNetwork {
	...
	_buildLayer(layer, inputsNumber, neuronsNumber) {
		...
		for (let ii = 0; ii < neuronsNumber; ++ii) {
			...
			for (let jj = 0; jj < inputsNumber; ++jj) {
				...
				// the range of this value is => [-1 < value < +1]
				const randomRangedValue = Math.random() - Math.random();

				connectionWeights.push(randomRangedValue);
			}

			layer.push(connectionWeights);
		}
	}
	...
	_processLayer(layer, inputs, output) {
		...
		// Cycle over all the connectionWeights and sum their weights against the inputs.
		for (let ii = 0; ii < layer.length; ++ii) {

			const connectionWeights = layer[ii];

			let activation = 0.0;

			// Sum the weights to the activation value.
			for (let jj = 0; jj < inputs.length; ++jj)
				activation += inputs[jj] * connectionWeights[jj];
	...
	set weights(arr_weights) {
		...
		for (let kk = 0; kk < this._hiddenLayers.length; ++kk) {
			...
			for (let ii = 0; ii < layer.length; ++ii) {
				...
				for (let jj = 0; jj < connectionWeights.length; ++jj)
					connectionWeights[jj] = arr_weights[weights_inc++];
			}
		}

		for (let ii = 0; ii < this._outputLayer.length; ++ii) {

			const connectionWeights = this._outputLayer[ii];
	...
	get weights() {
		...
		for (let ii = 0; ii < this._hiddenLayers.length; ++ii) {
			...
			for (let jj = 0; jj < layer.length; ++jj) {
				...
				const connectionWeights = layer[jj];

				for (let kk = 0; kk < connectionWeights.length; ++kk)
					out_weights.push(connectionWeights[kk]);
			}
		}

		for (let jj = 0; jj < this._outputLayer.length; ++jj) {

			const connectionWeights = this._outputLayer[jj];
